...
Chena_raw <- read.csv("2025/flat_data/Chena Sonar Reviewed_2025.csv", stringsAsFactors=F)
...
#### doing the same data steps with the raw (non-reviewed) sonar data

# reading data, incorporating some fixes to ensure that the standardize_ functions won't break

Chena_raw_nonreviewed <- read.csv("2025/flat_data/RAW Chena Sonar_2025.csv", stringsAsFactors=F)

names(Chena_raw_nonreviewed)
summary(Chena_raw_nonreviewed)
head(Chena_raw_nonreviewed)


Chena_all_nonreviewed <- Chena_raw_nonreviewed %>%
  rename(Date1 = Date.1) %>%
  rename(L_cm = Length) %>%
  mutate(Date1 = ifelse(!is.na(Date), Date, Date1)) %>% # Date1 seems to be the master date column
  # mutate(Date1 = as.Date(Date1, format="%m/%d/%Y"))
  mutate(Date1 = as.Date(Date1))
names(Chena_all_nonreviewed)
...
# all_fish <- subset(all_sonar, !is.na(length))
all_fish_nonreviewed <- subset(all_sonar_nonreviewed, length>=400)
...
lengthdata_raw <- read.csv("2025/flat_data/AllCarcassData_upto2025.csv",
...
Chena_runtiming <- read.csv(file="2025/flat_data/Chena_historic_2025.csv")
...
Salcha_runtiming <- read.csv(file="2025/flat_data/Salcha_historic_2025.csv")
