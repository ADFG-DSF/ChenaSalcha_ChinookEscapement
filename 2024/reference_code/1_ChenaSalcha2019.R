...
Chena_South <- read.csv("ChenaSouth.csv", stringsAsFactors=F)
Chena_North <- read.csv("ChenaNorth.csv", stringsAsFactors=F)
Salcha_South <- read.csv("Salchagravelside.csv", stringsAsFactors=F)    
Salcha_North <- read.csv("Salchatowerside.csv", stringsAsFactors=F)     



## Some minor data fixage so the standardize_ functions below won't break.

names(Chena_North)[names(Chena_North)=="Date.1"] <- "Date1"
names(Salcha_South)[names(Salcha_South)=="Date.1"] <- "Date1"
names(Salcha_North)[names(Salcha_North)=="Date.1"] <- "Date1"

names(Chena_North)[names(Chena_North)=="Length"] <- "L_cm"
...
all_sonar <- rbind(standardize_DIDSON(Chena_South, river="Chena", station="Chena South"),
                   standardize_ARIS(Chena_North, river="Chena", station="Chena North"),
                   standardize_ARIS(Salcha_South, river="Salcha", station="Salcha South"),
...
lengthdata <- read.csv("AllCarcassData_upto2019.csv",header=T)
...
lengthdata$Length <- as.numeric(as.character(lengthdata$Length))
...
library(jagsUI)
ncores <- 3  # number of cores for parallel chains.  I use 6 on the big desktop, 3 for laptop
...
niter <- 50000   # 10k takes 7.7 minutes, 50k takes 35 minutes
ncores <- 3


tstart <- Sys.time()
print(tstart)
length.data <- list(y=lengthdata$Length, year=lengthdata$yearfac, cat=lengthdata$cat,
                    N=nrow(lengthdata), Ncat=max(lengthdata$cat), Nyear= max(lengthdata$yearfac))
length.jags.out <- jagsUI::jags(model.file="length.jags", data=length.data,
...
# # jags.names(length.jags.out,425)
# # 
# # par(mfrow=c(5,5))
# # # jags.trace(length.jags.out)
# # jags.trace(length.jags.out,which.param = 1:8)
# # jags.trace(length.jags.out,which.param = 18:25)
# 
prior_mn <- length.jags.out$mean$catmean
prior_se <- length.jags.out$sd$catmean
prior_sd_mn <- length.jags.out$mean$sigma
prior_sd_sd <- length.jags.out$sd$sigma
# 
mu_mn <- length.jags.out$mean$mu
mu_sd <- length.jags.out$sd$mu
...
Chena_runtiming <- read.csv(file="Chena_historic_2019.csv")
...
Salcha_runtiming <- read.csv(file="Salcha_historic_2019.csv")
...
niter <- 500000   # 100k runs in 6 min, 500k in 30
ncores <- 3

tstart <- Sys.time()
print(tstart)
CS.jags.out <- jagsUI::jags(model.file="CS.jags",data=CS.data, parameters.to.save=c("b0","b1","a0","a1","rho0","rho1"),
                            n.chains=ncores, parallel=T, n.iter=niter, n.thin=niter/2000, n.burnin=niter/2)
Sys.time()-tstart

## running the model again, taking out the 2019 Salcha data

CS.data_alt <- CS.data
CS.data_alt$Schin[CS.data_alt$Syear==CS.data_alt$nyear] <- NA
CS.jags.out_alt <- jagsUI::jags(model.file="CS.jags",data=CS.data_alt, parameters.to.save=c("b0","b1","a0","a1","rho0","rho1"),
                            n.chains=ncores, parallel=T, n.iter=niter, n.thin=niter/2000, n.burnin=niter/2)


## saving point estimates for regression parameters

a0 <- CS.jags.out$mean$a0
a1 <- CS.jags.out$mean$a1
b0 <- CS.jags.out$mean$b0
b1 <- CS.jags.out$mean$b1

a0_alt <- CS.jags.out_alt$mean$a0
a1_alt <- CS.jags.out_alt$mean$a1
b0_alt <- CS.jags.out_alt$mean$b0
b1_alt <- CS.jags.out_alt$mean$b1

# save(CS.jags.out, length.jags.out, lengthdata, file="priorouts.Rdata")
...
length.jags.out_df <- as.data.frame(as.matrix(length.jags.out$samples))
CS.jags.out_df <- as.data.frame(as.matrix(CS.jags.out$samples))
dim(length.jags.out_df)
dim(CS.jags.out_df)

par(mfrow=c(5,2))
trace_df(CS.jags.out_df)
...
par(mfrow=c(3,2))
hist(unlist(CS.jags.out$Rhat))
hist(unlist(CS.jags.out$n.eff))
hist(unlist(CS.jags.out_alt$Rhat))
hist(unlist(CS.jags.out_alt$n.eff))
hist(unlist(length.jags.out$Rhat))
hist(unlist(length.jags.out$n.eff))

sapply(CS.jags.out$Rhat, function(x) sum(x>1.1, na.rm=T))
sapply(CS.jags.out$n.eff, function(x) sum(x<500, na.rm=T))
sapply(length.jags.out$Rhat, function(x) sum(x>1.1, na.rm=T))
sapply(length.jags.out$n.eff, function(x) sum(x<500, na.rm=T))

sapply(CS.jags.out$Rhat, function(x) which(x>1.1))
sapply(CS.jags.out$n.eff, function(x) which(x<500))
